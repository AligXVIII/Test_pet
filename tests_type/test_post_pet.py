...
def test_create_pet(log_response,log_request):
    with allure.step("Задаем данные для создания питомца"):
        ...
        log_request(METHOD, f"{PET_SERVICE_URL}/pet", request_json)
    ...
    with allure.step("Проверяем успешность обработки запроса"):
        ...
        log_response(post_response, METHOD, f"{PET_SERVICE_URL}/pet")

        assert post_response.status_code == 200


@allure.title("Создание питомца с неверно заданным ID")
def test_try_create_pet_with_invalid_id(log_response,log_request):
