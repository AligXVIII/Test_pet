...
def test_successful_get_pet_id(create_and_delete_pet, log_response):
    ...
    with allure.step("Отправляем запрос get и проверяем успешность запроса"):
        ...
        log_response(get_response, METHOD, f"{PET_SERVICE_URL}/pet/{pet_id}")

        assert get_response.status_code == 200

    with allure.step(
        "Проверяем,что имя питомца,которого мы создали совпадает с полученной информацией от запроса"
    ):
...
def test_get_by_name_and_check_failed(create_and_delete_pet, log_response):
    ...
    with allure.step("Получаем ошибку, что сервер не смог обработать запрос"):
        ...
        log_response(get_response, METHOD, f"{PET_SERVICE_URL}/pet/{pet_name}")

        assert get_response.status_code == 400


@allure.title("Неудачное получение информации о питомце с несуществующим ID")
def test_get_pet_by_nonexistent_id(log_response):
    ...
    with allure.step(
        "Отправляем запрос на несуществующий ID и получаем ошибку:Питомец не найден"
    ):

        get_response = requests.get(f"{PET_SERVICE_URL}/pet/{nonexistent_id}")

        log_response(get_response, METHOD, f"{PET_SERVICE_URL}/pet/{nonexistent_id}")
